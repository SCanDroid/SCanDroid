...
import com.ibm.wala.classLoader.Literal;
...
import com.ibm.wala.dex.instructions.Instruction.Visitor;
...
//              boolean isFloat = instruction.getType().equals(TYPE_double) || instruction.getType().equals(TYPE_float);
                emitInstruction(insts.BinaryOpInstruction(instruction.getOperator(), false, instruction.isUnsigned(), result, val1, val2, !instruction.isFloat()));
            }
...
//              boolean isFloat = instruction.getType().equals(TYPE_double) || instruction.getType().equals(TYPE_float);
                if (instruction.isSub())
                    emitInstruction(insts.BinaryOpInstruction(instruction.getOperator(), false, instruction.isUnsigned(), result, val2, val1, !instruction.isFloat()));
                else
                    emitInstruction(insts.BinaryOpInstruction(instruction.getOperator(), false, instruction.isUnsigned(), result, val1, val2, !instruction.isFloat()));
            }
...
//              workingState.push(result);
//              TypeReference t = instruction.getType();
                emitInstruction(insts.CheckCastInstruction(result, val, instruction.type, instruction.isPEI()));
            }
...
//                  symbol = symbolTable.getConstant(((Constant.ClassConstant)instruction).value);
                    ...
                    emitInstruction(insts.LoadMetadataInstruction(symbol, TypeReference.JavaLangClass, ((Constant.ClassConstant)instruction).value.getReference()));
...
//          }
            ...
            public void visitGetField(GetField instruction) {
                ...
                if(instruction instanceof GetField.GetInstanceField)
                {
                    ...
                    emitInstruction(insts.GetInstruction(result, instance, f));
                }
...
//              workingState.push(result);
//              TypeReference t = ShrikeUtil.makeTypeReference(loader, instruction.getType());
                emitInstruction(insts.InstanceofInstruction(result, ref, instruction.type));
            }
...
//              int ref = workingState.pop();
                emitInstruction(insts.MonitorInstruction(ref, instruction.enter));
            }
...
//              } else {
                    emitInstruction(insts.NewInstruction(result, instruction.newSiteRef));
//                  popN(instruction);
//              }
                workingState.setLocal(dest, result);
//              workingState.push(result);
            ...
            public void visitNewArray(NewArray instruction)
            {
                ...
                emitInstruction(insts.NewInstruction(result, instruction.newSiteRef, sizes));
                workingState.setLocal(dest, result);
            }
...
//                  boolean isPrimitive = symbolTable.isLongConstant(result) || symbolTable.isDoubleConstant(result);
                    ...
                    emitInstruction(insts.ReturnInstruction(result, isPrimitive));
...
//              int val = workingState.pop();
            ...
            public void visitThrow(Throw instruction) {
                ...
                emitInstruction(insts.ThrowInstruction(throwable));
...
//              workingState.push(result);
                if(instruction.isConversion())
                {
                    ...
                    emitInstruction(insts.ConversionInstruction(result, val, fromType, toType, overflows));
                }
                else
                {
